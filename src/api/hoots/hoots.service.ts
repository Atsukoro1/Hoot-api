...
export class HootsService {
    ...
    async search(body : ISearchReq) : Promise<IHootResponse> {
        ...
        // @ts-ignore
        const result = await this.hootsModel.paginate({ $text: { $search: body.query }}, options);
    ...
    async get(body): Promise<IHootResponse> {
        const response = await this.hootsModel.findById(body.id)
    ...
    async delete(body): Promise<IHootResponse> {
        ...
        if (existingReplyHoot && existingReplyHoot.author == body.userId) {
            const oppositePost = await this.hootsModel.findById(existingReplyHoot.isReplyTo);

            var index = oppositePost.replies.indexOf(existingReplyHoot._id.toString());
            ...
            return new HootResponse(true, undefined, existingReplyHoot);
             ...
             await this.hootsRepliesModel.findOneAndDelete({ isReplyTo: existingHoot._id });
        ...
        return new HootResponse(false, "No hoot has been found or you do not have permissions to remove it...", undefined);
    }
    ...
    async react(body): Promise<IHootResponse> {
        ...
        if (existingReplyHoot) {
            if (!existingReplyHoot.hearts.find(el => el == body.userId)) existingReplyHoot.hearts.push(body.userId);
            await existingReplyHoot.save();
            return new HootResponse(true, undefined, existingReplyHoot);

        } else if (existingHoot) {
            if (!existingHoot.hearts.find(el => el == body.userId)) existingHoot.hearts.push(body.userId);
            await existingHoot.save();
            return new HootResponse(true, undefined, existingHoot);
        }
