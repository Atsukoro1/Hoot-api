import { Controller, Post, Delete, Patch, Put, Body, Query, Get } from "@nestjs/common";
import { ICreateReq, IGetReq, IDeleteReactionReq, IDeleteReq, IEditReq, IHootResponse, IReactReq, HootResponse, ISearchReq } from "./hoots.interfaces";
import { validate, getSchema, reactSchema, searchSchema, createSchema, deleteSchema, deleteReactionSchema, editSchema } from "./hoots.validators";
...
export class HootsController {
    ...
    @Delete("/reactions")
    async removeReaction(@Body() body: IDeleteReactionReq): Promise<IHootResponse> {
    ...
    @Get("/search") 
    async searchHoots(@Body() body, @Query() query : ISearchReq) : Promise<IHootResponse> {
