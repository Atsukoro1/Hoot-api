...
import { IChangeReq, IUserSettingsResponse, UserSettingsResponse, IDeleteReq } from "./user.settings.interfaces"
...
export class UserSettingsService {
    ...
    async change(body: IChangeReq) : Promise<IUserSettingsResponse> {
        ...
        if(!match) return new UserSettingsResponse(false, "Passwords doesn't match!");

        if(body.newEmail) {
            const existingMailUser = await this.userModel.findOne({ email: body.newEmail });
            if(existingMailUser) return new UserSettingsResponse(false, "User with this email already exists!");
