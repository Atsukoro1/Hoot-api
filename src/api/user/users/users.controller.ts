...
import { validate, searchSchema, getFollowsSchema, getProfileSchema } from "./users.validators";
...
export class UsersController {
    ...
    @Get("/search")
    async searchUsers(@Body() body, @Query() query : ISearchRequest) : Promise<IUsersResponse> {
    ...
    @Get("/profile")
    async fetchProfile(@Body() body, @Query() query : IProfileRequest) : Promise<IUsersResponse> {
        query.userId = body.userId;
