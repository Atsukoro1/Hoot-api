...
import { IUsersResponse, UsersResponse, ISearchRequest, IProfileRequest } from "./users.interfaces";

@Injectable()
export class UsersService {
    constructor(
        @InjectModel("User") private userModel: Model<User>,
        @InjectModel("Relationships") private relationshipsModel: Model<Relationships>,
    ...
    async searchUsers(body : ISearchRequest) : Promise<IUsersResponse> {
        ...
        // @ts-ignore
        const result = await this.userModel.paginate({ username: { "$regex": body.query, "$options": "i" } }, options);

        return new UsersResponse(false, undefined, result);
    }

    async fetchProfile(body : IProfileRequest) : Promise<IUsersResponse> {
        var user = await this.userModel.findById(body.userId, {}, { select: "-password -ua" });

        body.page ? body.page : body.page = 1;
        var idsToGet : string[] | Array<object> = user.hoots.splice((body.page - 1) * 10, 10);
    ...
    async fetchFollow(body, type) {
        ...
        if(type == "following") {
            // @ts-ignore
            result = await this.relationshipsModel.paginate({ from: body.userId, type: 1 }, options);
        } else {
            // @ts-ignore
            result = await this.relationshipsModel.paginate({ to: body.userId, type: 1 }, options);
        }
